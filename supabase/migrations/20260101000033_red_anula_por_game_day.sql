-- La anulación por roja ahora se imputa al DÍA DE JUEGO de la tarjeta (game_day),
-- no a su fecha de creación. Así una roja ATRASADA (emitida hoy para una jornada
-- anterior) anula el día correcto aunque ese partido se recalcule. Respaldo a
-- created_at para tarjetas viejas sin game_day. Resto de la función sin cambios.
      ...
      WHERE p.match_id = p_match_id
        AND EXISTS (
            ...
            WHERE s.user_id = p.user_id
              ...
              AND s.active = TRUE
              AND date_trunc('day', (COALESCE(s.game_day, s.created_at) AT TIME ZONE 'America/La_Paz') - INTERVAL '3 hours') = v_gameday
        );
